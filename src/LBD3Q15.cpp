...
void LatticeBolztmannD3Q15::Colission(){
  ...
  rhos = this->rho();
  Ux = (this->Jx()/rhos)*this->ConditionsU ;
  Uy = (this->Jy()/rhos)*this->ConditionsU ;
  Uz = (this->Jz()/rhos)*this->ConditionsU ;
  
  array feq = this->feq3( rhos, Ux , Uy , Uz);

  for( int i = 0 ; i < q ; i++){
    fs[i] += beta*( feq(span,span,span,i) - fs[i]);
  }
  ...
  */
  ...
  array nns = array ( q , nums);
  gfor( seq i , q ){
    fs[ nns(i).scalar<int>() ] =  fs[ nns(i).scalar<int>() ] + rex*( feq2(  nns(i).scalar<int>() , rho,  Ux , Uy , Uz ) - fs[  nns(i).scalar<int>() ] ); 
  }
  */
  //af_print( fs[ 1 ] );
}
...
void LatticeBolztmannD3Q15::Adveccion(){
   ...
   gfor( seq i, 15 ){
    ...
    //fs[ nns(i).scalar<int>() ] =  af::shift( fp , vel_x(i).scalar<int>() ,vel_y(i).scalar<int>()  ,vel_z(i).scalar<int>() , 0  );
    this->fs[1] =  af::shift( fp , 1 , 0  ,0 , 0  );
   }
   af_print( this->fs[1] );
  /*
  */
...
void LatticeBolztmannD3Q15::Print(void){
  ...
  SetConditions( Ux , Uy, Uz );
  int *h_ux = this->ConditionsU.host<int>();
  ...
  for(int y = 0 ; y< Ly  ;y++)  {
     for(int z = 0 ; z < Lz ; z++)
	{
	  int ux = h_ux[x + Ly*(y + Lx*z) ], uy = h_uy[x + Ly*(y + Lx*z) ], uz =  h_uz[x + Ly*(y + Lx*z) ];
	  if ( ux !=  0  ) continue;
	  std::cout << x << " "<< y << " " << z << " " << ux << std::endl;	
